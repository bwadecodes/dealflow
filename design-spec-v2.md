...
| `/dealflow-super-analyst` | Propose + execute structured Excel analysis from source data |
...
`openpyxl`-based helper for skills that produce structured Excel output (`/dealflow-super-analyst`, `/dealflow-cohort`, `/dealflow-returns`, `/dealflow-termsheet`, `/dealflow-process`, `/dealflow-prescreen`).
...
`~/.claude/dealflow/firm-style.yaml`. Built by `/dealflow-firmstyle`, read by every skill that produces narrative or branded output.
...
templates:
  ic_memo: ~/.claude/dealflow/templates/ic-memo.md
  prescreen: ~/.claude/dealflow/templates/prescreen.md
  termsheet: ~/.claude/dealflow/templates/termsheet.md
  model_layout: ~/.claude/dealflow/templates/model-layout.md
brand:
...
- Apollo MCP for team/hiring/contact data (if user has Apollo connected)
...
**Dependencies:** Web access, PDF author, optional Apollo MCP.

---

### `/dealflow-super-analyst`

**Purpose:** Look at raw data, propose interesting analyses, execute the chosen ones as auditable Excel.
...
**Outputs:**
- `reports/analysis-YYYY-MM-DD.xlsx`
...
- Stress test: run downside scenarios on the model (revenue miss 10/20/30%, margin compression, multiple compression at exit, key customer loss). Surface what breaks.
...
8. **`/dealflow-super-analyst`** + **`/dealflow-cohort`**
